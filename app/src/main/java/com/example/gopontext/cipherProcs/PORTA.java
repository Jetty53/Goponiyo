...
public class PORTA {
    private String encodeDecodePORTA(String inputText, String keyText){
        ...
        //System.out.println(secInputText);
        ...
        for (int i = 0; i < secInputText.length(); i++){
            ...
            if (charToNum(keyText.charAt(k)) % 2 != 0){
                num = 13 - (((charToNum(keyText.charAt(k)))-1)/2);
            }else{
                num = 13 - ((charToNum(keyText.charAt(k)))/2);
            }

            //System.out.println(k + " " + num);

            if (charToNum(secInputText.charAt(i)) >= 0 && charToNum(secInputText.charAt(i)) <= 12){

                cipCharInt = ((charToNum(secInputText.charAt(i))) + num) % 13;

            }else if(charToNum(secInputText.charAt(i)) >= 13 && charToNum(secInputText.charAt(i)) <= 25){
    ...
    public String encodeDecodeProcedurePORTA(String inputText, String key) {

        return encodeDecodePORTA(inputText, key);
